...
class GetUserPiecesCommand(private val ddcCliConfigFile: DdcCliConfigFile) : Runnable {
    ...
    override fun run() {
        ...
        val fromAsString = if (from != null) from.toString() else ""
        val toAsString = if (to != null) to.toString() else ""

        if (decrypt) {
            getUserPiecesDecrypted(configOptions, ddcConsumer, userPubKey, fromAsString, toAsString, fields)
        } else {
            getUserPiecesEncrypted(ddcConsumer, userPubKey, fromAsString, toAsString, fields)
        }
    ...
    private fun getUserPiecesDecrypted(
        ...
        ddcConsumer.getUserPieces(userPubKey, from, to, fields).subscribe().with(
            { piece ->
                try {
                    piece.data =
    ...
    private fun getUserPiecesEncrypted(
        ...
        ddcConsumer.getUserPieces(userPubKey, from, to, fields)
